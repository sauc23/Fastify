...
const nm = new Nekomori();

const chat = async (msg = "Hello", assistantId = 286) => {
    return await nm.chat(msg, assistantId);
};

const search = async (query, useTags = false) => {
    return await nm.search(query, useTags);
};

const tags = async (tagIndex = 0, byIndex = 5) => {
    return await nm.tags(tagIndex, byIndex);
};

export { chat, search, tags };
