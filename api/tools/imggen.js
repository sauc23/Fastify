...
class ImgGen {
  ...
  async getStatus(uuid) {
    return await this.fetchData(`${this.baseUrl}${this.endpoints.watchProcess}/${uuid}`, {
      method: "GET"
    });
  }
  async gen(prompt) {
  ...
  async genImg(links, type, options = {}) {
    try {
    const mediaBuffer = await this.getBuffer(links);
...
export const genImg = async (links, type, options = {}) => {
    try {
        return await imgGen.genImg(links, type, options);
    } catch (error) {
...
export const gen = async (prompt) => {
    try {
        return await imgGen.gen(prompt);
    } catch (error) {
